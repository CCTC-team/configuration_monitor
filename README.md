...
[![Configuration Monitor EM Cypress Tests](https://github.com/CCTC-team/configuration_monitor/actions/workflows/cypress-tests.yml/badge.svg)](https://github.com/CCTC-team/configuration_monitor/actions/workflows/cypress-tests.yml)
...
4. Injects this EM into `CCTC_REDCap_Docker/redcap_source/modules/configuration_monitor_v1.0.0` and brings the Docker stack up (`app`, `db`, `mailhog`).
...
6. Builds the spec list from `automated_tests/E.129.*.feature` (excluding `*REDUNDANT*`) and runs them via `npm run test:retry-failed` (up to 3 attempts per spec, Chrome).
...
- `EM_NAME` / `EM_VERSION` — `configuration_monitor` / `v1.0.0`. Bump `EM_VERSION` when releasing a new module version so the spec glob and inject path stay aligned.
