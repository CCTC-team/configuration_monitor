Feature: E.129.100 - The system shall support the ability to enable/disable Configuration Monitor external module.

  As a REDCap end user
  I want to see that Configuration Monitor is functioning as expected

  Scenario: E.129.100 - Enable external module - Default settings
    ...
    When I create a new project named "E.129.100.100" by clicking on "New Project" in the menu bar, selecting "Practice / Just for fun" from the dropdown, choosing file "redcap_val/Project_redcap_val_nodata.xml", and clicking the "Create Project" button
    And I should NOT see a link labeled exactly "Manage"
    And I logout

  Scenario: E.129.200 - Enable module on all projects by default
    Given I login to REDCap with the user "Test_Admin"
    When I click on the link labeled "Control Center"
  ...
  Scenario: E.129.300 - Make module discoverable by users
    Given I login to REDCap with the user "Test_Admin"
    When I click on the link labeled "Control Center"
    And I click on the link labeled exactly "Manage"
    Then I should see "External Modules - Module Manager"
  ...
  Scenario: E.129.400 - Allow non-admins to enable this module on projects
    Given I login to REDCap with the user "Test_Admin"
    When I click on the link labeled "Control Center"   
    And I click on the link labeled exactly "Manage"
    Then I should see "External Modules - Module Manager"
    ...
    Given I login to REDCap with the user "Test_User1"
    When I create a new project named "E.129.100.400" by clicking on "New Project" in the menu bar, selecting "Practice / Just for fun" from the dropdown, choosing file "redcap_val/Project_redcap_val_nodata.xml", and clicking the "Create Project" button
    And I click on the link labeled exactly "Manage"
    Then I should see "External Modules - Project Module Manager"
    And I should NOT see "Configuration Monitor - v1.0.0"
  ...
  Scenario: E.129.500 - Hide this module from non-admins in the list of enabled modules on each project
    ...
    And I check the checkbox labeled "Enable User Role Changes"
    And I click on the button labeled "Save"
    Then I should see "Configuration Monitor - v1.0.0"
    And I logout
  ...
  Scenario: E.129.600 - View Usage of the external module
    When I click on the link labeled "Control Center"
    And I click on the link labeled exactly "Manage"
    Then I should see "Configuration Monitor - v1.0.0"
    When I click on the button labeled "View Usage"
    Then I should see a link labeled "E.129.100.400" in the dialog box
    And I should see a link labeled "E.129.100.500" in the dialog box
    And I should see a link labeled "E.129.100.600" in the dialog box
    When I click on the link labeled "E.129.100.400" in the dialog box
    # # For Request Activation of project E.129.100.300 which doesn't work in ATS.
    # # Can be checked manually
    # And I should see a link labeled "E.129.100.300" in the dialog box
    Then I should see "Project Home"
    And I should see "E.129.100.400"
    And I logout
